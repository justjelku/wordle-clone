...
import { dailyWords, users, gameStats } from "./schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  getTodayTopPatterns(
    ...
    Array<{ username: string; guesses: string[]; guessCount: number }>
  >;
...
export class MemStorage implements IStorage {
  ...
  async getUserStats(userId: string): Promise<UserStats> {
    ...
    const averageGuesses =
      gamesWon.length > 0
        ? gamesWon.reduce((sum, game) => sum + game.guessCount, 0) /
          gamesWon.length
        : 0;
    const bestGuess =
      gamesWon.length > 0
        ? Math.min(...gamesWon.map((game) => game.guessCount))
        : 0;
  ...
  async getLeaderboard(limit = 10): Promise<LeaderboardEntry[]> {
    ...
    const leaderboard: LeaderboardEntry[] = Array.from(userStatsMap.values())
      .map((stats) => ({
        ...
        averageGuesses:
          stats.gamesWon > 0
            ? Math.round((stats.totalGuesses / stats.gamesWon) * 10) / 10
            : 0,
  ...
  async getTodayTopPatterns(
    date: string,
  ): Promise<
    Array<{ username: string; guesses: string[]; guessCount: number }>
  > {
    // Get all games for today
    const todayGames = this.gameStats.filter(
      (game) => game.date === date && game.completed === "won",
    );
...
export class DatabaseStorage implements IStorage {
  ...
  async hasUserCompletedToday(
    ...
    return game && (game.completed === "won" || game.completed === "lost")
      ? game
      : null;
  }
}
